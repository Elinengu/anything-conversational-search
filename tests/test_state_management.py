...
class PlanApplicationTests(unittest.TestCase):
    def test_stagnation_plan_changes_question_policy_but_respects_declines(self) -> None:
        ...
        # opening_track=None isolates this test to the STAGNATING override alone
        # (see TrackRoutingTests / dual_track_routing.md for the dual-track base
        # _policy_for_state layers on top of); with route_policies off or no
        # track, _policy_for(None) always falls back to agent.config.policy.
        self.assertIs(agent._policy_for_state(None, state, plan), targeted_policy)
        state.dead_attributes.add("other")
        self.assertIs(agent._policy_for_state(None, state, plan), default_policy)
