"""DeepSeek-backed state rewriting for context distillation.
...
The distiller falls back to regex-based extraction when DeepSeek is unavailable
...
class StateRewriteInstruction:
    """Parsed state rewrite directive from DeepSeek."""
    erase: list[str]
    keep: list[str]
    add: list[str]
...
def rewrite_state_for_override(
    ...
    """Use DeepSeek to determine what constraints to erase/keep/add after an override.
    ...
    # Only call DeepSeek if we have enough conversation to reason about
    ...
    try:
        ...
        # Defense in depth: DeepSeekClient.generate/generate_json already catch
        # network and parsing errors internally, but this call site guards
        # independently too, matching src/router.py's _llm_route_hint - a
        # caller here must never propagate an exception into the distiller.
        return None
