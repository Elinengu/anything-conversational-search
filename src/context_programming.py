...
class ContextDistiller:
    ...
    def distill(
        ...
        # If an override was detected, use Gemini to intelligently rewrite the state.
        # This determines which pre-override constraints to erase vs. keep.
        # Falls back to deterministic handling when Gemini is unavailable or quota is exhausted.
        if state.override_turn is not None and len(state.utterances) > state.override_turn:
            ...
            if rewrite is not None:
                ...
                # but Gemini's parsing helps us understand what to deprioritize.
